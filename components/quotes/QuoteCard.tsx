...
const QuoteCard: React.FC<QuoteCardProps> = ({ quote, isLiked, onLike }) => {
  const author = MOCK_USERS[quote.authorId] || MOCK_USERS['user-0'];
  ...
  return (
    <div className="group relative aspect-square overflow-hidden rounded-lg shadow-lg transform hover:scale-105 transition-transform duration-300 bg-gray-800 animate-fade-in-up">
        <img
          src={quote.imageUrl}
          alt={quote.quoteText}
      ...
      <div className="absolute bottom-2 right-2 flex items-center space-x-2">
        {quote.audioUrl && (
          <button onClick={handlePlayAudio} className="p-2 rounded-full bg-black/50 backdrop-blur-sm text-white hover:text-blue-400 transition-colors focus:outline-none" aria-label="Play audio">
        ...
        <button onClick={handleDownload} className="p-2 rounded-full bg-black/50 backdrop-blur-sm text-white hover:text-green-400 transition-colors focus:outline-none" aria-label="Download quote">
          <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M3 17a1 1 0 011-1h12a1 1 0 110 2H4a1 1 0 01-1-1zm3.293-7.707a1 1 0 011.414 0L9 10.586V3a1 1 0 112 0v7.586l1.293-1.293a1 1 0 111.414 1.414l-3 3a1 1 0 01-1.414 0l-3-3a1 1 0 010-1.414z" clipRule="evenodd" /></svg>
        </button>
         <button onClick={(e) => { e.stopPropagation(); onLike(quote.id); }} className="p-2 rounded-full bg-black/50 backdrop-blur-sm text-white hover:text-red-500 transition-colors focus:outline-none" aria-label="Like quote">
